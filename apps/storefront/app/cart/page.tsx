...
import { Alert, Button, Container, EmptyState, Input, SectionHeading, buttonClasses } from '@bobby/ui';
...
export default function CartPage() {
      ...
      <Container wide>
        <div className="py-16">
          <EmptyState
            ...
            action={
              <Link href="/collections/womens" className={buttonClasses({ variant: 'primary', size: 'lg' })}>
                Start shopping
              </Link>
            }
          />
  ...
  return (
    <div className="py-12">
      <Container wide>
        <SectionHeading title="Your Bag" eyebrow={`${totals.itemCount} items`} className="mb-10" as="h1" />
            ...
            {lines.map((line) => (
              <li key={line.variantId} className="flex gap-4 py-5 sm:gap-6">
                <Link
                  ...
                  className="relative aspect-2/3 w-24 shrink-0 overflow-hidden bg-cream-panel sm:w-28"
                >
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <Link href={`/products/${line.slug}`} className="text-sm text-ink hover:text-maroon">
                        {line.title}
                      </Link>
                      ...
                      {line.availableStock <= 3 && (
                        <p className="mt-1 text-xs text-warning">Only {line.availableStock} left</p>
                      )}
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="mt-auto flex flex-wrap items-center justify-between gap-3 pt-3">
                    <div className="flex items-center border border-line">
                      <button
                        ...
                        className="flex size-9 items-center justify-center hover:bg-cream-panel"
                      >
                      ...
                      <button
                        ...
                        className="flex size-9 items-center justify-center hover:bg-cream-panel disabled:opacity-40"
                      >
                    ...
                    <div className="text-right">
                      ...
                      {line.quantity > 1 && (
                        <p className="text-xs text-muted">{formatPaise(line.unitPricePaise)} each</p>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <aside aria-label="Order summary" className="lg:sticky lg:top-32 lg:self-start">
            <div className="border border-line bg-card p-6">
              ...
              <div className="mt-5 border-b border-line pb-5">
                {appliedCoupon ? (
                  <div className="flex items-center justify-between gap-2 bg-success-soft px-3 py-2.5">
              ...
              <Link
                href="/checkout"
                className={buttonClasses({ variant: 'primary', size: 'lg', fullWidth: true, className: 'mt-5' })}
